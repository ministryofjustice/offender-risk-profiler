...
abstract class IntegrationTestBase {
  ...
  fun `clear queues`() {
    ...
    prisonMockServer.stubPing()
    pathfinderMockServer.stubPing()
    ...
    riskProfilerChangeSqsClientSpy.purgeQueue(PurgeQueueRequest.builder().queueUrl(riskProfilerChangeQueueUrl).build())
      .get()
    riskProfilerChangeSqsDlqClientSpy.purgeQueue(PurgeQueueRequest.builder().queueUrl(riskProfilerChangeDlqUrl).build())
      .get()
  }

  private val riskProfilerChangeQueue by lazy {
    hmppsQueueService.findByQueueId("riskprofilechangequeue")
      ?: throw MissingQueueException("HmppsQueue outboundqueue not found")
  }
  ...
  fun HmppsSqsProperties.riskProfilerChangeQueueConfig() =
    queues["riskprofilechangequeue"]
      ?: throw MissingQueueException("riskprofilechangequeue has not been loaded from configuration properties")
  ...
  internal fun HttpHeaders.authToken(
    user: String?,
    roles: List<String> = listOf("ROLE_QUEUE_ADMIN", "ROLE_RISK_PROFILER"),
  ) {
    this.setBearerAuth(
  ...
  class SqsConfig(private val hmppsQueueFactory: HmppsQueueFactory) {
    ...
    fun riskProfileChangeQueueSqsClient(
      ...
      with(hmppsSqsProperties) {
        val config = queues["riskprofilechangequeue"]
          ?: throw MissingQueueException("HmppsSqsProperties config for hmppsriskprofilechangequeue not found")
    ...
    fun riskProfileChangeQueueSqsDlqClient(hmppsSqsProperties: HmppsSqsProperties): SqsAsyncClient =
      with(hmppsSqsProperties) {
        val config = queues["riskprofilechangequeue"]
          ?: throw MissingQueueException("HmppsSqsProperties config for outboundqueue not found")
